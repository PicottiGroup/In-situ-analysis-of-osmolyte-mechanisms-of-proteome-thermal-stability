...
# # logging setup for logfile
# logging.basicConfig(filename="Log_file.txt",
#         level=logging.DEBUG,
#         format='%(levelname)s: %(asctime)s %(message)s',
#         datefmt='%m/%d/%Y %I:%M:%S')

# # prevent logging output from matplot.pyplot
...
def FitGPs_function2(sm, path_in, trypticity,path_output,mll,sol):

    #common_path = "/Users/moni/Documents/PhD/Experiments/"
    #path = sys.argv[1] + "0" + np.array2string(path_list["Experiment"][path_list["sm"] == sm].unique()[0]) + "/" + sm + "/python_import/"
    #path_output = sys.argv[2] + "0" + np.array2string(path_list["Experiment"][path_list["sm"] == sm].unique()[0]) + "/" + sm + "/python_output/"

    #trypticity = "FT"
    #filename = sm + "_" + trypticity + ".csv"
    data = pd.read_csv(path_in)
    # data['y'] = np.log(data['y'] + 1)
    
    # Filer out peptides for which null and alternative model coincide or not exactly two different conditions exist (TODO to be extended to more than 2 conditions)
   ...
   # counter = 0
    ...
    for i in range(training_iterations):
        ...
        logging.info('Iter %d/%d - Loss: %.6f' % (i+1, training_iterations,loss.item()))
        ...
        if (i in range(1,26,1)) | (i in range(25,1000,25)):
            # Set into eval mode
            model.eval()
            likelihood.eval()
            ...
            mll2 = mll + "_" + str(i) + ".csv"

            #MLL_peptide.to_csv(mll2,sep=',', index=True, header=True)
            ...
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                ...
                sol2 = sol + "_" + str(i) + ".csv"

                df_joint.to_csv(sol2, sep=',', index=False, header=True)



        if i > 1 and abs(loss_hist[i] - loss_hist[i-1]) <= 0.000000011:
            break
